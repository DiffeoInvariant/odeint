...
int main(int argc, char** argv)
{
  ...
  const double tol = 1.0e-4, a = 16.0, r = 45.0, b = 4.0;
  const double dt_min=1.0e-6, dt_max=1.0, t0 = 0.0, tf = 100.0;
  ...
  /*
  const double a2 = 0.398;
  const double b2 = 2.0;
  const double c = 4.0;
  State rosx0{3.0, 1.0, 1.0};
  std::string filename{"test_rkf_rossler.csv"};
  const auto rhs = std::bind(Rossler, ph::_1, ph::_2, a2, b2, c);
  auto trajectory = odeint::rkfehlberg_integrate(rhs, rosx0, dt_min, dt_max, t0, tf, tol);
  odeint::write_csv(filename, trajectory);
  */
  return 0;
}
